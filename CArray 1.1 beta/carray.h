...
template <typename TData>
CArrayIterator<TData>::CArrayIterator(
    TData * _p
    ) : p(_p)
{
...
template <typename TData>
bool CArrayIterator<TData>::operator==(
	CArrayIterator const & _other
	) const
{
    return p == _other.p;
...
template <typename TData>
TData & CArrayIterator<TData>::operator*() const
{
    return *p;
}
...
template <typename TData>
CArrayIterator<TData> & CArrayIterator<TData>::operator++()
{
	++p;
	return *this;
}
...
template <typename TData>
CArrayIterator<TData> & CArrayIterator<TData>::operator++(int)
{
...
// Ïðåäëîæåíèå: âîçâðàùàòü óäàëÿåìûé ýëåìåíò ïîëüçîâàòåëþ ìàññèâà,
// ÷òîáû îí ñàì ðåøàë, êîãäà ðàçðóøèòü ýëåìåíò. Êîíòåéíåð õðàíèò îáúåêòû
// è íå äîëæåí èõ óíè÷òîæàòü.
template <typename TData>
